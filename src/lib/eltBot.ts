      ...
      try {
        ...
        const response = await ai.models.generateContent({
          ...
          contents: `
            ...
            ${studentTurnsCount === 0 ? `4. MISSING STUDENT TRANSCRIPT: The transcript contains NO [Student] lines because the user's device did not support text transcription, even though they spoke (the audio was processed separately). You MUST infer the conversation context from the Tutor's responses. Provide a helpful, encouraging report based on what the Tutor said, and explicitly mention in the overall assessment that their exact words couldn't be transcribed for detailed grammar analysis.` : ""}
